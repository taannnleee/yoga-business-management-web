...
const OrderManagement = () => {
    ...
    const fetchOrders = async () => {
      try {
        const response = await axiosInstance.get(
          `/api/admin/get-all-order-of-user-by-status/${selectedStatus}?sortBy=createdAt&sortDir=desc`,
        );
  ...
  const handleStatusChange = async (orderId: number, newStatus: string) => {
    try {
      const response = await axiosInstance.patch(`/api/admin/update-order-status/${orderId}`, {
        status: newStatus,
      });
